...
function ModelGallery({
  ...
  onDownload: (model: ARModel) => Promise<void>;
}) {
  ...
  const renderItem = useCallback(({ item, index }: { item: ARModel, index: number }) => {
    const modelId = String(item._id || item.id || (item as any).id);
    ...
    const isDownloaded = downloadedIds.has(modelId);
    const progress = downloadingProgress[modelId];
  ...
  }, [cardWidth, colors.card, downloadedIds, downloadingProgress, environment.emoji, gradientColors, modelPreviewHeight, onDownload, onOpenModel]);
...
function ARScreenContent() {
  ...
  // Scan local cache on focus/mount
  const scanCache = useCallback(async (allModels: ARModel[]) => {
    try {
      const downloaded = new Set<string>();
      for (const model of allModels) {
        const modelId = String(model._id || model.id || (model as any).id);
        const isLocal = await ARCache.isModelDownloaded(modelId);
        if (isLocal) {
      ...
      console.warn('Error scanning AR cache:', error);
    }
  }, []);

  // Sync cache state when models change or screen focus
  useFocusEffect(
    useCallback(() => {
      if (modelsQuery.data) {
        scanCache(modelsQuery.data);
      }
    }, [modelsQuery.data, scanCache])
  );

  const handleDownload = useCallback(async (model: ARModel) => {
    const modelId = String(model._id || model.id || (model as any).id);
    
    // Use the same priority logic as the viewer: check model.file/fileUrl first
    const remoteUrl = pickRemoteUrl(
      (model as any).file,
      (model as any).fileUrl,
      (model as any).modelFile,
      (model as any).model_url
    ) || ARService.getModelFileUrl(modelId);

    try {
      setDownloadingProgress(prev => ({ ...prev, [modelId]: 0 }));
      await ARCache.downloadModel(modelId, remoteUrl, (progress) => {
        setDownloadingProgress(prev => ({ ...prev, [modelId]: progress }));
      });
      setDownloadedIds(prev => new Set([...prev, modelId]));
    } catch (error: any) {
  ...
  const startActualScan = async (model: ARModel) => {
    ...
    try {
      ...
      // 1. Resolve Model and Metadata
      for (const candidateId of modelIdCandidates) {
        if (await ARCache.isModelDownloaded(candidateId)) {
          modelId = candidateId;
          break;
        }
      }
      ...
      for (const candidateId of modelIdCandidates) {
        if (modelId && candidateId === modelId) break; 
        try {
          ...
          try {
            const resp = await ARService.getUserArModalById(candidateId);
            const userModal = resp?.data?.modal || resp?.data?.arModal || resp?.data;
            const linkModel = userModal?.modelId;
            const linkedId = String((typeof linkModel === 'string' ? linkModel : '') || linkModel?._id || '').trim();
            if (!linkedId) continue;
            const hasFile = pickRemoteUrl(userModal?.file, userModal?.fileUrl, (linkModel as any)?.file, (linkModel as any)?.fileUrl);
            if (hasFile) {
              modelId = linkedId;
              resolvedModelRecord = userModal;
              resolvedNestedModel = typeof linkModel === 'object' ? linkModel : null;
              break;
            }
          } catch { /* skip */ }
        }
      ...
      if (!modelId) {
        Alert.alert('Model Missing', 'This model details could not be found.');
        return;
      }

      // -- Resolve URLs --
      const remoteModelUrl = pickRemoteUrl(
        resolvedModelRecord?.file, resolvedModelRecord?.fileUrl,
        resolvedNestedModel?.file, resolvedNestedModel?.fileUrl,
        modelRecord?.file, modelRecord?.fileUrl,
        nestedModel?.file, nestedModel?.fileUrl
      ) || ARService.getModelFileUrl(modelId);

      let modelFileUrl = remoteModelUrl;
      const cached = await ARCache.isModelDownloaded(modelId);
      if (cached) {
        modelFileUrl = `file://${ARCache.getLocalModelPath(modelId)}`;
      }

      const referenceImageUrl = pickRemoteUrl(
        resolvedModelRecord?.preview_image, resolvedModelRecord?.thumbnail, resolvedModelRecord?.previewUrl,
        resolvedNestedModel?.preview_image, resolvedNestedModel?.thumbnail, resolvedNestedModel?.previewUrl,
        modelRecord?.preview_image, modelRecord?.thumbnail, modelRecord?.previewUrl
      ) || ARService.getPreviewImageUrl(modelId);

      // -- Resolve Audios --
      let audiosJson: string | undefined;
      try {
        const modelAudios = resolvedModelRecord?.audios || resolvedNestedModel?.audios || modelRecord?.audios || nestedModel?.audios || [];
      ...
      const modelDisplayName = resolvedNestedModel?.name || resolvedModelRecord?.name || nestedModel?.name || modelRecord?.name || 'model';

      try {
        console.log('[ARScanner] Launch attempt 1:', modelFileUrl);
        await ARScannerModule.startScannerDynamic(modelFileUrl, referenceImageUrl, modelDisplayName, audiosJson);
      } catch (error: any) {
        if (modelFileUrl.startsWith('file://')) {
          console.warn('[ARScanner] Local fail, trying network:', remoteModelUrl);
          await ARScannerModule.startScannerDynamic(remoteModelUrl, referenceImageUrl, modelDisplayName, audiosJson);
        } else {
          throw error;
        }
      }
    } catch (error: any) {
      Alert.alert('AR Error', error?.message || 'Failed to initialize the scanner.');
    } finally {
      closeSheet();
    }
  };
